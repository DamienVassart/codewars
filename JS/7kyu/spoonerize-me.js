...
Spoonerisms can be more complex. 
...
function spoonerize(words) {
    let firstVowels = words.split(' ').map(str => str.slice(0, 1)); // replace 1 by str.search(/[aeiouy]/gi) to make it work with any kind of spoonerism
    let truncated = words.split(' ').map(str => str.slice(1)); // replace 1 by str.search(/[aeiouy]/gi) to make it work with any kind of spoonerism
    return firstVowels.reverse().map((str, i) => str.concat(truncated[i])).join` `;
}