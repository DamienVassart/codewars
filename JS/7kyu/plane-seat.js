...
the naming system consists of a number (in this case between 1-60) 
that denotes the section of the plane where the seat is (1-20 = front, 21-40 = middle, 40+ = back). 
...
function planeSeat(a){
	const row = +a.replace(/[A-Za-z]/gi, '');
	const col = a.replace(/\d+/, '');
	if (row > 60 || (/[IJL-Z]/gi).test(col)) return 'No Seat!!';
	return (row <= 20 && 'Front' || (row > 20 && row <= 40) && 'Middle' || row > 40 && 'Back') + '-' + ((/[A-C]/i).test(col) && 'Left' || (/[D-F]/i).test(col) && 'Middle' || (/[GHK]/i).test(col) && 'Right');
}